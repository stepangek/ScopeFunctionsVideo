...
class ApplyDemo {
    // apply -  возвращет обьект, на котором была применена. Часто используется для "донастройки"
    // различных обьектов, как это показано в 2 примере. Так же данная особенность позволяет использовать apply как
